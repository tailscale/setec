...
func TestACL(t *testing.T) {
	rules := acl.Rules{
		...
		acl.Rule{
			Action: []acl.Action{acl.ActionInfo, acl.ActionPut, acl.ActionSet, acl.ActionActivate},
			Secret: []acl.Secret{"*"},
		},
		acl.Rule{
	...
	tests := []testCase{
		...
		allow("set", "control/foo"),
		allow("set", "control/bar"),
		allow("set", "control/quux"),
		allow("set", "something/else"),
		allow("set", "dev/foo"),
